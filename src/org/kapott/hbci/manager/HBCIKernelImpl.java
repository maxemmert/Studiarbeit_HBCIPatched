...
public final class HBCIKernelImpl implements HBCIKernel {
    ...
    public String getHBCIVersion(int dummy) {
        String ret = "";
        Element ret1 = XMLStorage.getHashMap().get("MsgHeadUser");
        NodeList nodes = ret1.getElementsByTagName("value");
     ...
     * 
     * This method initializes some internal variables that are needed when you later set parameters using the rawSet() method.
     * 
     * @param name Is the name of the message to be created. You can find a list of all possible names in the xml-file describing the syntax, all nodes <MSGdef
     * id="xxx"> contain descriptions for the message with name "xxx"
     */
    ...
    /*
     * Sets a parameter for the currently to be created message (mid-level API).
     * 
     * The @p key is the path to the element to be set, with the message name of the current message removed, e.g. if you want to set the value of
     * DialogInit.Idn.prodName, you have to specify Idn.prodName as @key.
     * 
    ...
    public void rawSet(String key, String value) {
        HBCIUtils.log("setting raw property " + currentMsgName + "." + key + " to \"" + value + "\"", HBCIUtils.LOG_DEBUG2);
     ...
     * 
     * This method creates the message specified earlier by the methods rawNewJob() and rawSet(), signs and encrypts it using the values of @p inst, @p user, @p
     * signit and @p crypit and sends it to server.
     * 
     * After that it waits for the response, decrypts it, checks the signature of the received message and returns a Properties object, that contains as keys
     * the pathnames of all elements of the received message, and as values the corresponding value of the element with that path
     * 
     * bricht diese methode mit einer exception ab, so muss die aufrufende methode die nachricht komplett neu erzeugen.
     * 
     ...
     * 
     * @return A Properties object that contains a path-value-pair for each dataelement of the received message.
     */
    public HBCIMsgStatus rawDoIt(HBCIPassportList passports, boolean signit, boolean cryptit, boolean needSig, boolean needCrypt) {
        ...
        try {
            ...
            /*
             * zu jeder SyntaxElement-Referenz (2:3,1)==(SEG:DEG,DE) den Pfad des jeweiligen Elementes speichern
             */
            ...
            /*
             * für alle Elemente (Pfadnamen) die aktuellen Werte speichern, wie sie bei der ausgehenden Nachricht versandt werden
             */
            ...
            if (currentMsgName.startsWith("DialogEnd") && HBCIUtils.getParam(paramName, "no").equals("yes")) {
                HBCIUtils.log(e, HBCIUtils.LOG_WARN);
                HBCIUtils.log("error while receiving DialogEnd response - " + "but ignoring it because of special setting", HBCIUtils.LOG_WARN);
            } else {
                ret.addException(e);
            }
        } finally {
